...
class Project(models.Model): #ПРОЕКТ, ДЛЯ КОТОРОГО ЗАПИСАНЫ ЗАМЕТКИ
    ...
    repository_link = models.URLField(max_length=200, verbose_name="ссылка на репозиторий")
    users = models.ManyToManyField(get_user_model(), verbose_name="пользователи, работающие с проектом") # Много пользователей - много проектов.
...
class ToDo(models.Model): #ЗАМЕТКА
    ...
    author = models.ForeignKey(get_user_model(), on_delete=models.CASCADE, verbose_name="автор заметки") # Одна заметка - один автор.
